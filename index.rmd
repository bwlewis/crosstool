...
option menu and an HTML span element. The select option is not useful here,
but only used to illustrate another HTML control element.
...
slider = crosstool(s, "transmitter", "<input type='range' min='2' max='11', value='2'/>", width=200, init="2")
sel =  sprintf("<select>%s</select>", paste(Map(function(i) sprintf("<option value='%d'>%d</option>", i, i), 2:11), collapse=""))
opt = crosstool(s, "receiver", sel, value="value", height=100, width=200)
...
p1 = tags$div(list(tags$h3("view-only select"), opt))
p2 = tags$div(list(tags$h3("an HTML SPAN element with looked up values from the slider:") , span))
p3 = tags$div(list(tags$h3("an HTML SPAN with transcoded transceiver values") , span2, relay))
