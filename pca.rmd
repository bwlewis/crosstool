...
Ding and He strengthen and clarify previously known results and introduce a
few new ideas, in particular:
...
lower-dimensional ones and recover full-fidelity cluster information.  And
thanks to algorithms like the IRLBA<sup>2</sup> and randomized SVD<sup>6</sup>
...
problems from different viewpoints using the tools of network analysis, which
we explore a bit in the example below.

Please note that these ideas say nothing about the computational complexity of
k-means, a known NP-hard problem. The result simply says that we can conduct
...
million identified genomic variants. The 1000 Genomes Project has collected
full genome sequences for more than 2,504 individuals. That is, if we simply
record that zero means an person does not contain a particular variant, and a
one means that a person does contain the variant, then the 1000 Genomes Project
variant data can be expressed as a 2,504-row by more than 81 million-column
sparse matrix of zeros and ones. That's high dimensional!
...
But what I really like is Ding and He's idea about connectivity analysis.
Their approach assigns a connectivity coefficient between 0 (not at all
likely to be in the same group) and 1 (very likely to be in the same group)
to each pair of observations, or _vertices_ in the network visualization
below.


The neat thing about this idea is that it lets us explore connected communities
and how they evolve as we allow the connections to become weaker. The visualization
...
What I find fascinating about the last paragraph is that some cheap network
community algorithms might produce excellent starting points for heuristic
k-means implementations.

I also love that, by explicitly illustrating connections between observations,
the network visualization lets us see paths _between_ clusters. Sure, this is
kind of analogous points in the scatter plot lying in-between two cluster
centers, but it's easier to see in the network visualization.
...
6. Halko, Nathan, Per-Gunnar Martinsson, and Joel A. Tropp. "Finding structure with randomness: Stochastic algorithms for constructing approximate matrix decompositions." (2009).

## The example

Click on a vertex in the network visualization to identify it. Adjust the slider
to change the network connectivity coefficent.
...
p = c(0.999, 0.995, 0.99, 0.97, 0.95, 0.925, 0.9, 0.85)
g = Map(f, p)
...
# Crazy, right?
relay1 = crosstool(sd1, "transceiver", relay=sd3, width=0, init="0", lookup=cv, channel="filter->select")
# Relay the slider values directly to the graph
relay2 = crosstool(sd1, "transceiver", relay=sd2, width=0, channel="filter")
# Relay graph selections to the d3 scatter plot
relay3 = crosstool(sd2, "transceiver", relay=sd3, width=0)
